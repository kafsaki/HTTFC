...
class MyDataset(Dataset):
    def __init__(self, ImagePath, LabelPath, word2idx, dataSetName, isTrain=False, transform=None):
        ...
        if dataSetName == "CE-CSL" or "CE-CSL-mediapipe":
            ...
            imgs = []
            imageSeqPath = os.path.join(ImagePath)  # this points to a specific subfolder like 'test-00001'
            imageName = os.path.basename(imageSeqPath)  # 'test-00001'
            print(f"imageSeqPath： {imageSeqPath}")
            print(f"imageName： {imageName}")
            try:
                imgs.append((imageSeqPath, lable[imageName]))  # 路径和标签添加到列表中
            except:
                print(f"xkf except: imageName:{imageName} 需要将当前imageName存入test.csv（注意存入时，不能添加已有词语之外的词语，因为训练的模型预测的词语id总数是固定的）")
            print(f"imgs： {imgs}")
